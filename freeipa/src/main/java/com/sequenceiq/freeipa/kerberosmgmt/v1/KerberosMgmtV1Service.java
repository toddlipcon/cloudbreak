...
public class KerberosMgmtV1Service {
    ...
    public void deleteServicePrincipal(String environmentCrn, String serverHostName, String serviceName, String clusterCrn, String accountId)
            throws FreeIpaClientException, DeleteException {
        Stack freeIpaStack = getFreeIpaStack(environmentCrn, accountId);
        String realm = getRealm(freeIpaStack);
        String canonicalPrincipal = constructCanonicalPrincipal(serviceName, serverHostName, realm);
        FreeIpaClient ipaClient = freeIpaClientFactory.getFreeIpaClientForStack(freeIpaStack);
    ...
    public void deleteHost(String environmentCrn, String serverHostName, String clusterCrn, String accountId) throws FreeIpaClientException, DeleteException {
        Stack freeIpaStack = getFreeIpaStack(environmentCrn, accountId);
        FreeIpaClient ipaClient = freeIpaClientFactory.getFreeIpaClientForStack(freeIpaStack);

        Set<String> services = ipaClient.findAllService().stream()
                .filter(s -> s.getKrbprincipalname().contains(serverHostName))
        ...
        delHost(serverHostName, ipaClient);
        VaultPathBuilder b = new VaultPathBuilder()
                .withOptionalClusterCrn()
                .withAccountId(accountId)
                .withEnvironmentCrn(environmentCrn)
